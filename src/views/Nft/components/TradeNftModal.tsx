...
import { TradingDataItem } from '../contexts/NftProvider'
...
const TransferNftModal: React.FC<TradeNftModalProps> = ({
  nft,
  approvedAddress,
  tradingData,
  isOwner,
  ...
  const handleConfirm = async () => {
    try {
      if (!isOwner) {
        ...
        await nftSaleContract.methods
          .putOnSale(nft.bunnyId, salePrice)
          .send({
            from: account,
          })
          .then(() => {
  ...
  const approveNft = async () => {
    try {
      await nftContract.methods
        .approve(NFT_SALE_ADDRESS, nft.bunnyId)
        .send({
          from: account,
        })
        .then(() => {
  ...
  if (process.env.REACT_APP_DEBUG === 'true') console.log(tradingData.ownerPrice, 'trading data on modal')
                  ...
                  {isOnSale ? (
                    <>
                      ...
                      <Button
                        ...
                        variant="subtle"
                        size="sm"
                        style={{ borderRadius: '5px', marginTop: '5px', width: '33px', height: '33px', padding: '1px' }}
                        data-tip
                        data-for="doggFace"
                      >
                        <FaTrashAlt style={{ fill: 'white' }} />
                      </Button>
                    </>
                  ) : (
                    <></>
